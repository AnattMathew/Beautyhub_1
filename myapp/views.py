...
@login_required(login_url='user_login')  # Ensure the user is logged in
def add_to_cart(request, product_id):
    try:
        # Retrieve the product, raise 404 if not found
        product = Product.objects.get(pk=product_id)
    except Product.DoesNotExist:
        return redirect('product_not_found')  # Redirect to a page or return 404 if product not found

    if request.method == 'POST':
        if product.quantity > 0:  # Check if the product is in stock
            # Create or update cart item (assuming Cart model has a user, product, and quantity)
            cart_item = Cart.objects.create(user=request.user, product=product, quantity=1)  # Adjust quantity as needed
            cart_item.save()
            ...
            # Redirect to the cart page or wherever you manage cart items
            return redirect('cart')  # Ensure 'cart' matches the URL name for your cart view
        else:
            # Handle out of stock case
            return render(request, 'out_of_stock.html', {'product': product})
    
    # For GET requests, return the product detail page or another valid response
